...
import PurchaseModal from './PurchaseModal'; // Importamos el modal de compra
...
const EventDetailModal = ({ open, onClose, evento }) => {
  ...
  // ESTADOS PARA CONTROLAR LA COMPRA
  ...
  const handleOpenPurchase = (lote, cat) => {
    setSelectedTicket({ loteName: lote.loteName, category: cat });
  ...
  return (
    <>
                        ...
                        <AccordionSummary expandIcon={isLoteActive ? <FiChevronDown color="#FF6B00" /> : <FiLock color="#666" />}>
                          <Box className="accordion-summary-content">
                            <Typography className="lote-name-tech">
                              {lote.loteName} {!isLoteActive && <span className="sold-out-tag">LOTE_OFFLINE</span>}
                            </Typography>
                            {isLoteActive && (
                              ...
                              return (
                                <div key={cat._id} className={`ticket-item-minimal-dark ${!isAvailable ? 'cat-disabled' : ''}`}>
                                  ...
                                  <Button 
                                    ...
                                    onClick={() => handleOpenPurchase(lote, cat)}
      ...
      {/* MODAL DE COMPRA REGISTRO */}
      <PurchaseModal 
        open={purchaseOpen} 
        onClose={() => setPurchaseOpen(false)}
        eventName={evento.name}
        loteName={selectedTicket?.loteName}
