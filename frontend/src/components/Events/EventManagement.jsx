...
import { Container, Typography, CircularProgress, Box, useMediaQuery, useTheme, Button } from '@mui/material';
import { FiPlus, FiActivity, FiEdit3, FiInfo, FiTrash2, FiArrowLeft } from 'react-icons/fi';
...
const EventManagement = () => {
  ...
  // Estado para alternar entre la lista de eventos y el Dashboard POS
  const [currentView, setCurrentView] = useState('list'); // 'list' | 'dashboard'
  const [activeDashboardEventId, setActiveDashboardEventId] = useState('ALL');
  ...
  // Función al hacer clic en el botón INFO (DATA)
  const handleOpenDashboard = (eventId) => {
    setActiveDashboardEventId(eventId);
      ...
      <Container maxWidth="xl" sx={{ mt: 4 }}>
        {currentView === 'dashboard' ? (
          <Box>
            ...
            {/* INTEGRACIÓN DEL DASHBOARD PRESENCIAL */}
            <TicketSalesDashboard 
              events={events}
              salesHistory={[]} // Aquí puedes pasar tu hook/estado con el historial de ventas
              onProcessSale={async (salePayload) => {
                console.log("Procesando venta:", salePayload);
                // Lógica de procesamiento de venta presencial (API call)
              }}
            />
          </Box>
        ) : (
                ...
                {events.map((event) => (
                  <div key={event._id} className="event-card-industrial">
                    ...
                    <Box className="card-action-bar-integrated">
                      ...
                      {/* DISPARADOR AL DASHBOARD POS */}
                      <button onClick={() => handleOpenDashboard(event._id)} className="action-btn-sub info">
